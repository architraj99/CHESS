...
function renderCapturedPanel(color) {
    const panel = capturePanelFor(color);
    const captured = capturedBy(color);
    const pawnVictimColor = color === "white" ? "black" : "white";
    const capturedPawns = captured.filter((piece) => piece.type === "pawn").sort((first, second) => fileLetters.indexOf(first.pawnFile) - fileLetters.indexOf(second.pawnFile));
    ...
    pawnNamesElement.textContent = `Pawns: ${capturedPawns.length ? capturedPawns.map((piece) => 
    `${piece.pawnFile}-pawn`).join(", ") : "none"}`;
}

function renderCapturedPanels() {
    renderCapturedPanel("white");
    renderCapturedPanel("black");
}
